...
- **Legacy JWT secret проекта отзывать нельзя.** Проект подписывает свои токены асимметричным ES256 (это видно в `/auth/v1/.well-known/jwks.json`), но HS256-проверка остаётся включённой — на ней держится и штатный `anon`-ключ, и наш `discord-auth`, который выпускает токен с claim `can_edit`. Если legacy-секрет отозвать на странице Settings → JWT Keys, права редактирования отвалятся, а приватного ES256-ключа Supabase наружу не отдаёт, так что подписать самим будет нечем.
